function [res, iteration, f_hist, iter_hist] = dfo_tr(bb_func, x_initial, clist, options)

  f_hist = [];

  iter_hist = zeros(6,100*size(x_initial,1));
  ...
  while 1
    ...
    %[20241021-LHT]
    h = 0.01;
    df_k = zeros(size(xtrial,1),1);
    ddf_k = zeros(size(xtrial,1),size(xtrial,1));
    f_tmp_i = zeros(size(xtrial,1),1);
    for i = 1:size(xtrial,1)
        e_i = zeros(size(xtrial,1),1);
        e_i(i,1) = h;
        f_tmp_i(i) = bb_func(xtrial + e_i);
    end
    for i = 1:size(xtrial,1)
        e_i = zeros(size(xtrial,1),1);
        e_i(i,1) = h;
        % 计算\nablda f(xk)
        % df_k(i,1) = (bb_func(xtrial + e_i) - bb_func(xtrial - e_i)) / (2*h);
        df_k(i,1) = (f_tmp_i(i) - bb_func(xtrial - e_i)) / (2*h);
        for j = i:size(xtrial,1)
            e_j = zeros(size(xtrial,1),1);
            e_j(j,1) = h;
            % 计算\nablda^2 f(xk)
            % ddf_k(i,j) = (bb_func(xtrial + e_i + e_j) - bb_func(xtrial + e_i) - bb_func(xtrial + e_j) + ftrue) / (h * h);
            ddf_k(i,j) = (bb_func(xtrial + e_i + e_j) - f_tmp_i(i) - f_tmp_i(j) + ftrue) / (h * h);
            ddf_k(j,i) = ddf_k(i,j);
        end
    end
    
    iter_hist(1, iteration + 1) = delta;

    g_tmp = (H * s + g);
    iter_hist(2, iteration + 1) = norm(g_tmp - df_k, 2);%||\nablda m(xk)- \nablda f(xk)||_2
    iter_hist(3, iteration + 1) = norm(H - ddf_k,2);%||\nablda^2 m - \nablda^2 f(xk)||_2

    vec1 = inv(H) * g_tmp;
    vec2 = inv(ddf_k) * df_k;
    iter_hist(4, iteration + 1) = norm(vec1 - vec2, 2);%||(\nablda^2 m)^(-1)\nablda m(xk)- (\nablda^2 f(xk))^(-1)\nablda f(xk)||_2
    
    iter_hist(5, iteration + 1) = dot(g_tmp, df_k) / (norm(g_tmp, 2) * norm(df_k, 2));% iter2的夹角
    iter_hist(6, iteration + 1) = dot(vec1, vec2) / (norm(vec1, 2) * norm(vec2, 2));% iter4的夹角
