...
function [X, F, flag, xkin, ihist] = ...
    pounders(fun, dfun, X0, n, npmax, nfmax, gtol, clist, delta, nfs, m, F0, xkin, L, U, printf, spsolver, hfun, combinemodels)

ihist = zeros(nfmax,9);%用于存储过程量 [LHT-20241101]
...
while nf < nfmax
    ...
    elseif spsolver == 2 % Arnold Neumaier's minq5
        ...
        ihist(nf, 9) = dot(vec3, vec4) / (norm(vec3, 2) * norm(vec4, 2));% 夹角
