...
function [X, F, flag, xkin, ihist] = ...
    poundersC(fun, dfun, X0, n, npmax, nfmax, gtol, clist, delta, nfs, m, F0, xkin, L, U, printf, spsolver, hfun, combinemodels)

ihist = zeros(nfmax,9);% [20241101]
...
[flag, X0, npmax, F0, L, U] = ...
    checkinputss(fun, X0, n, npmax, nfmax, gtol, clist, delta, nfs, m, F0, xkin, L, U);
...
while nf < nfmax
    ...
    diffxpc1 = X(Mind, :) - X(previousxkin, :);
    modelxpc1=diffxpc1 * previousG + 0.5 * sum((diffxpc1 * previousH) .* diffxpc1, 2) + Cres;
    % diffxpc2 = X(nf-np:nf-1, :) - X(previousxkin, :);
    % modelxpc2=diffxpc2 * previousG + 0.5 * sum((diffxpc2 * previousH) .* diffxpc2, 2) + Cres;

    %check

    F(Mind)- modelxpc1;
    % F(nf-np:nf-1)- modelxpc2
 ...
 % if np>=npmax
 if nf >= 2 &&exist('previousxkin', 'var') % if 1
    % diffxpc3 = X(Mind, :) - X(xkin, :);%previous
    diffxpc3 = X(Mind, :) - X(previousxkin, :);%

    % 3


    % modelxpc3=diffxpc3 * Gres + 0.5 * sum((diffxpc3 * (previousH+Hresdel)) .* diffxpc3, 2) + Cres;
    % F(Mind)- modelxpc3;
...
clist;
    ...
    modelxpc4=diffxpc3 * previousG + 0.5 * sum((diffxpc3 * previousH) .* diffxpc3, 2) + Cres;
    F(Mind)- modelxpc4;

    nf;

    modelxpcshift=diffxpc3 * (previousG + previousH * (X(xkin,:)-X(previousxkin,:))' )+ 0.5 * sum((diffxpc3 * previousH) .* diffxpc3, 2) + Cres;
    F(Mind)- modelxpcshift;
...
% nf
  ...
  % 6
 %    diffxpc5 = X(Mind, :) - X(xkin, :);
 % modelxpc5=diffxpc5 * Gres + 0.5 * sum((diffxpc5 * (previousH+Hresdel)) .* diffxpc5, 2) + F(xkin);
 %    F(Mind)- modelxpc5;
    ...
    % if norm(F(Mind) - modelxpc6,1) > 1e-10
...
nf;
  ...
  clist;
        ...
        [Hresdel_mat, Gres] = Cquad_frob(X(Mind,:), F(Mind)- modelxpc4, delta, X(xkin,:), clist);

        % Cquad_frob 1

        GresC=Gres;

        % Gres = previousG +Gres; correctonce

        Gres = previousG + previousH * (X(xkin,:)-X(previousxkin,:))'+ Gres;
        ...
        Hresdel = Hresdel_mat;
            ...
            diffxpc6 = X(Mind, :) - X(xkin, :);

% 8

 modelxpc6=diffxpc6 * Gres + 0.5 * sum((diffxpc6 * (previousH+Hresdel)) .* diffxpc6, 2) + F(xkin);
    F(Mind)- modelxpc6;
...
%        ihist(nf, 3) = norm(g_tmp - df, 2);%||\nablda m(xk)- \nablda f(xk)||_2
%        ihist(nf, 4) = dot(g_tmp, df) / (norm(g_tmp, 2) * norm(df, 2));%||\nablda m(xk)- \nablda f(xk)||_2的夹角%

%        ihist(nf, 5) = norm(H - ddf, 2);%||\nablda^2 m - \nablda^2 f(xk)||_2%

%        vec1 = inv(H) * g_tmp;
%        vec2 = inv(ddf) * df;
%        ihist(nf, 6) = norm(vec1 - vec2, 2);%||(\nablda^2 m)^(-1)\nablda m(xk)- (\nablda^2 f(xk))^(-1)\nablda f(xk)||_2
%        ihist(nf, 7) = dot(vec1, vec2) / (norm(vec1, 2) * norm(vec2, 2));%||(\nablda^2 m)^(-1)\nablda m(xk)- (\nablda^2 f(xk))^(-1)\nablda f(xk)||_2的夹角
%        
%        % (8) ||(\nablda^2 m)^(-1)\nablda m(xk) * min{1,Delta_k/||(\nablda^2 m)^(-1)\nablda m(xk)||}- (\nablda^2 f(xk))^(-1)\nablda f(xk)* min{1,Delta_k/||(\nablda^2 f(xk))^(-1)\nablda f(xk)||}||_2
%        % ||vec1 * min{1,Delta_k/||vec1||}- vec2* min{1,Delta_k/||vec2||}||_2
%        vec3 = vec1 * min(1,delta/norm(vec1,2));
%        vec4 = vec2 * min(1,delta/norm(vec2,2));
%        ihist(nf, 8) = norm(vec3 - vec4, 2);
%        ihist(nf, 9) = dot(vec3, vec4) / (norm(vec3, 2) * norm(vec4, 2));% 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
