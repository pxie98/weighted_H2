...
if size(G, 2) ~= n
    ier = -1;
    disp('minq: Hessian has wrong dimension');
end
...
if ~all(all(abs(G - G') <= 1e-12))
    ...
    % G-G'
    ier = -1;
    disp('minq: Hessian is not symmetric');
end
if size(c, 1) ~= n || size(c, 2) ~= 1
    ier = -1;
    disp('minq: linear term has wrong dimension');
end
if size(xu, 1) ~= n || size(xu, 2) ~= 1
    ier = -1;
    disp('minq: lower bound has wrong dimension');
end
if size(xo, 1) ~= n || size(xo, 2) ~= 1
    ier = -1;
    disp('minq: upper bound has wrong dimension');
end
if exist('xx') == 1
    if size(xx, 1) ~= n || size(xx, 2) ~= 1
        ier = -1;
        disp('minq: starting point has wrong dimension');
    end
end
if ier == -1
    x = NaN + zeros(n, 1);
...
while 1
    ...
    elseif nitref > nitrefmax
        % good termination
        if prt
            disp('terminate: nitref>nitrefmax');
        end
        ier = 0;
        break
